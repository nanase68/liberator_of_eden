<?php require("./common/common.php")?>
<!DOCTYPE html>
<html lang="ja">
<head>
	<title><?php echo PAGE_TITLE?>:TOPページ</title>
	<?php require("./parts/common_head.php");?>
	<link rel="stylesheet" href="<?php echo CSS_PATH?>theme_upload.css">
	<script src="./js/jquery.raty/jquery.raty.js"></script>
	<script src="./js/jquery.raty/jquery.raty.set.js"></script>

</head>
<body>
<div id="wrapper">

	<?php require("./parts/common_header.php");?>
	<?php require("./parts/common_menu.php");?>
	<?php require("./parts/theme_upload_content.php");?>
	<?php require("./parts/common_footer.php");?>
</div>
<?php //require("./parts/common_haguruma.php");?>
</body>
</html>